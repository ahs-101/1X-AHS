...
const Footer: React.FC<FooterProps> = ({ leftLinks, rightLinks, copyrightText, barCount = 23 }) => {
  ...
  return (
    <footer ref={footerRef} className="bg-black text-white relative flex flex-col w-full h-full justify-between lg:h-screen select-none">
      <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-4 pb-24 pt-8 px-4">
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, i) => (
              <li key={i}>
                <a href={link.href} className="text-sm hover:text-white/60 transition-colors">{link.label}</a>
              </li>
            ))}
          </ul>
          <p className="text-sm mt-4 flex items-center gap-x-1 text-white/40">
        ...
        <div className="space-y-4">
          ...
          <div className="text-right mt-4">
            <button
              className="text-sm hover:underline inline-flex items-center text-white/40 hover:text-white transition-colors"
        ...
        <div style={{ marginTop: 0 }}>
          {Array.from({ length: barCount }).map((_, index) => (
            <div
              ...
              style={{ height: `${index + 1}px`, backgroundColor: "rgb(255, 255, 255)", transition: "transform 0.1s ease", willChange: "transform", marginTop: "-2px" }}
            />
          ))}
        </div>
      </div>
